...
export function buildMapObstacle(zone: MapObstacle, map: StadiumMap, terrain: MapTerrain): THREE.Group {
  ...
  if (zone.style==='single-tree') {
    ...
    for (let i=0;i<trunks;i++) {
      // Keep each crown legible as its own tree. Wide angle jitter and crowns
      // sized from the whole grove radius previously made neighbouring trees
      // intersect heavily, especially in Viridian's four-tree groves.
      const angle=i*Math.PI*2/trunks+(random()-0.5)*0.2;
      const spread=r*(0.48+random()*0.06);
      ...
      const canopy=mesh(prop,new THREE.IcosahedronGeometry(r*(0.2+random()*0.035),1),material(leaves[Math.floor(random()*leaves.length)]),x,height+r*0.18,z);
      canopy.scale.set(1+random()*0.1,1.15+random()*0.25,1+random()*0.1);
