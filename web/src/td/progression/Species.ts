...
export const SPECIES: Record<string, SpeciesDef> = {
  ...
  // ---- Course Titans ------------------------------------------------------
  // Each course's final ends on one of these (docs/match-length.md). They are
  // caught like any creep, so each needs a line of its own.
  weedle: {
    id: 'weedle', deployCost: 100, expYield: 52, role: 'THE STINGER',
    forms: [form('Weedle', 'Bug', 0, 35, 50, 20, 'Poison'), form('Kakuna', 'Bug', 7, 25, 35, 25, 'Poison'),
      form('Beedrill', 'Bug', 10, 80, 75, 45, 'Poison')],
    basicAttack: 'poison_sting',
    paths: [
      { id: 'swarm', label: 'SWARM', tiers: [
        tier('Fury Attack', 120, undefined, 'Stings 2 enemies at once.',
          { kind: 'multishot', count: 2 }),
        tier('Twineedle', 240, 8, 'The attack becomes twin barbs that hit harder.',
          { kind: 'replaceAttack', moveId: 'twineedle' }),
        tier('Pin Missile', 410, 20, 'Stings 3 enemies at once. Unlocks Pin Missile.',
          { kind: 'multishot', count: 3 }, { kind: 'signature', signatureId: 'pin_missile' }),
      ] },
      { id: 'venom', label: 'VENOM', tiers: [
        tier('Poison Barb', 110, undefined, 'Poison lands more often and lasts longer.',
          { kind: 'onHitStatus', status: 'poison', chance: 0.35, duration: 6 }),
        tier('Toxic Spikes', 230, 8, 'When a poisoned enemy faints, the poison jumps to the nearest one.',
          { kind: 'seedJump', radius: 3.5 }),
        tier('Venoshock', 400, 20, 'Hits 25% harder. Unlocks Toxic.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'toxic_sting' }),
      ] },
      { id: 'focus', label: 'FOCUS', tiers: [
        tier('Focus Energy', 120, undefined, '15% of hits are critical, for double damage.',
          { kind: 'crit', chance: 0.15, multiplier: 2 }),
        tier('Rage', 240, 8, 'Each hit on the same target deals 10% more, up to 6 times.',
          { kind: 'rage', perStack: 0.1, maxStacks: 6 }),
        tier('Fell Stinger', 410, 20, '25% of hits are critical. Unlocks Fell Stinger.',
          { kind: 'crit', chance: 0.25, multiplier: 2 }, { kind: 'signature', signatureId: 'fell_stinger' }),
      ] },
    ],
    createModel: () => M.createZubat(),
    description: 'A worm with a stinger, then a cocoon, then a swarm on its own.',
  },
  staryu: {
    id: 'staryu', deployCost: 130, expYield: 106, role: 'THE PRISM',
    forms: [form('Staryu', 'Water', 0, 45, 85, 70), form('Starmie', 'Water', 26, 75, 115, 100, 'Psychic')],
    basicAttack: 'water_gun',
    paths: [
      { id: 'star', label: 'STAR', tiers: [
        tier('Swift', 120, undefined, 'The attack becomes star rays that never miss.',
          { kind: 'replaceAttack', moveId: 'swift' }),
        tier('Rapid Spin', 240, 8, 'Attacks 30% faster.',
          { kind: 'scale', rate: 1.3 }),
        tier('Cosmic Power', 420, 26, 'Fires at 3 enemies at once. Unlocks Swift.',
          { kind: 'multishot', count: 3 }, { kind: 'signature', signatureId: 'swift_storm' }),
      ] },
      { id: 'psy', label: 'PSY', tiers: [
        tier('Psybeam', 130, undefined, 'The attack becomes a long psychic beam.',
          { kind: 'replaceAttack', moveId: 'psybeam_snipe' }),
        tier('Light Screen', 250, 8, 'Can aim at Phantoms.',
          { kind: 'seePhantoms' }),
        tier('Psychic', 430, 26, 'The beam hits 25% harder. Unlocks Psychic.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'star_psychic' }),
      ] },
      { id: 'tide', label: 'TIDE', tiers: [
        tier('BubbleBeam', 120, undefined, 'The attack becomes a bubble beam that pierces.',
          { kind: 'replaceAttack', moveId: 'bubblebeam' }),
        tier('Hydro Surge', 240, 8, 'The beam hits 30% harder.',
          { kind: 'scale', damage: 1.3 }),
        tier('Hydro Pump', 420, 26, 'The beam reaches 20% further. Unlocks Hydro Pump.',
          { kind: 'scale', range: 1.2 }, { kind: 'signature', signatureId: 'star_hydro' }),
      ] },
    ],
    createModel: () => M.createRattata(),
    description: 'A spinning star whose core glows like a gem.',
  },
  zapdos: {
    id: 'zapdos', deployCost: 260, expYield: 216, role: 'THE STORMBIRD',
    forms: [form('Zapdos', 'Electric', 0, 90, 100, 125, 'Flying')],
    basicAttack: 'thundershock',
    paths: [
      { id: 'thunder', label: 'THUNDER', tiers: [
        tier('Thunderbolt', 150, undefined, 'The attack becomes a heavier bolt.',
          { kind: 'replaceAttack', moveId: 'thunderbolt' }),
        tier('Charge', 280, 8, 'Bolts hit 30% harder and jump to 1 more enemy.',
          { kind: 'scale', damage: 1.3 }, { kind: 'chain', count: 1 }),
        tier('Thunder', 480, 30, 'Bolts jump to 2 more enemies. Unlocks Thunder.',
          { kind: 'chain', count: 2 }, { kind: 'signature', signatureId: 'zapdos_thunder' }),
      ] },
      { id: 'static', label: 'STATIC', tiers: [
        tier('Static', 140, undefined, 'Hits may paralyze.',
          { kind: 'onHitStatus', status: 'paralyze', chance: 0.2, duration: 1.5 }),
        tier('Discharge', 270, 8, 'Paralysis spreads to enemies next to the target.',
          { kind: 'spreadStatus', radius: 3 }),
        tier('Zap Cannon', 460, 30, 'Paralysis lands more often. Unlocks Zap Cannon.',
          { kind: 'onHitStatus', status: 'paralyze', chance: 0.35, duration: 2 }, { kind: 'signature', signatureId: 'zapdos_wave' }),
      ] },
      { id: 'drill', label: 'DRILL', tiers: [
        tier('Drill Peck', 150, undefined, 'The attack becomes a Heavy corkscrew peck.',
          { kind: 'replaceAttack', moveId: 'drill_peck' }),
        tier('Agility', 280, 8, 'Attacks 30% faster.',
          { kind: 'scale', rate: 1.3 }),
        tier('Lightning Rod', 470, 30, 'Pecks hit 25% harder. Unlocks Agility.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'zapdos_agility' }),
      ] },
    ],
    createModel: () => M.createBossTitan('Gyarados'),
    description: 'The legendary bird of thunder, nested in the Power Plant.',
  },
  moltres: {
    id: 'moltres', deployCost: 260, expYield: 217, role: 'THE FIREBIRD',
    forms: [form('Moltres', 'Fire', 0, 100, 90, 125, 'Flying')],
    basicAttack: 'ember',
    paths: [
      { id: 'flare', label: 'FLARE', tiers: [
        tier('Flamethrower', 150, undefined, 'The attack becomes a cone of fire.',
          { kind: 'replaceAttack', moveId: 'flamethrower' }),
        tier('Heat Wave', 280, 8, 'Flames hit 30% harder and reach further.',
          { kind: 'scale', damage: 1.3, range: 1.1 }),
        tier('Sky Blaze', 480, 30, 'Flames hit 20% harder. Unlocks Fire Blast.',
          { kind: 'scale', damage: 1.2 }, { kind: 'signature', signatureId: 'sky_blaze' }),
      ] },
      { id: 'trail', label: 'TRAIL', tiers: [
        tier('Fire Spin', 140, undefined, 'Every 4th attack leaves a burning patch on the lane.',
          { kind: 'hazard', hazard: 'fire_spin_patch', everyNth: 4 }),
        tier('Wildfire', 270, 8, 'Burning patches drop every 3rd attack.',
          { kind: 'hazard', hazard: 'fire_spin_patch', everyNth: 3 }),
        tier('Inferno', 460, 30, 'Burns land on every hit. Unlocks Inferno.',
          { kind: 'onHitStatus', status: 'burn', chance: 1, duration: 4 }, { kind: 'signature', signatureId: 'moltres_inferno' }),
      ] },
      { id: 'wing', label: 'WING', tiers: [
        tier('Wing Attack', 140, undefined, 'The attack becomes a sweeping wing strike.',
          { kind: 'replaceAttack', moveId: 'wing_attack' }),
        tier('Aerial Ace', 270, 8, '20% of hits are critical, for double damage.',
          { kind: 'crit', chance: 0.2, multiplier: 2 }),
        tier('Sky Attack', 470, 30, 'Strikes hit 25% harder. Unlocks Sky Attack.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'moltres_sky_attack' }),
      ] },
    ],
    createModel: () => M.createBossTitan('Gyarados'),
    description: 'The legendary bird of fire, seen above Victory Road.',
  },
  articuno: {
    id: 'articuno', deployCost: 260, expYield: 215, role: 'THE FROSTBIRD',
    forms: [form('Articuno', 'Ice', 0, 85, 85, 125, 'Flying')],
    basicAttack: 'ice_shard',
    paths: [
      { id: 'frost', label: 'FROST', tiers: [
        tier('Mist', 150, undefined, 'Enemies in range move 20% slower.',
          { kind: 'slowAura', slow: 0.2 }),
        tier('Powder Snow', 280, 8, 'Enemies in range move 35% slower.',
          { kind: 'slowAura', slow: 0.35 }),
        tier('Blizzard', 480, 30, 'Enemies in range move 45% slower. Unlocks Blizzard.',
          { kind: 'slowAura', slow: 0.45 }, { kind: 'signature', signatureId: 'articuno_blizzard' }),
      ] },
      { id: 'beam', label: 'BEAM', tiers: [
        tier('Ice Beam', 150, undefined, 'The attack becomes a freezing beam.',
          { kind: 'replaceAttack', moveId: 'ice_beam' }),
        tier('Deep Freeze', 280, 8, 'The beam hits 15% harder.',
          { kind: 'scale', damage: 1.15 }),
        tier('Ice Beam+', 470, 30, 'The beam reaches 20% further. Unlocks Ice Beam.',
          { kind: 'scale', range: 1.2 }, { kind: 'signature', signatureId: 'articuno_ice_beam' }),
      ] },
      { id: 'wing', label: 'WING', tiers: [
        tier('Drill Peck', 150, undefined, 'The attack becomes a Heavy corkscrew peck.',
          { kind: 'replaceAttack', moveId: 'drill_peck' }),
        tier('Mirror Coat', 280, 8, 'Hits Titans and elites 40% harder.',
          { kind: 'bonusVs', target: 'boss', multiplier: 1.4 }),
        tier('Air Cutter', 470, 30, 'Pecks hit 25% harder. Unlocks Air Cutter.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'articuno_air_cutter' }),
      ] },
    ],
    createModel: () => M.createBossTitan('Gyarados'),
    description: 'The legendary bird of ice, roosting in the Seafoam Islands.',
  },
  mewtwo: {
    id: 'mewtwo', deployCost: 320, expYield: 220, role: 'THE APEX',
    forms: [form('Mewtwo', 'Psychic', 0, 110, 130, 154)],
    basicAttack: 'confusion',
    paths: [
      { id: 'mind', label: 'MIND', tiers: [
        tier('Psy Burst', 170, undefined, 'The attack becomes a psychic shockwave around it.',
          { kind: 'replaceAttack', moveId: 'psy_ring' }),
        tier('Psych Up', 300, 8, 'Shockwaves hit 40% harder and reach 30% further.',
          { kind: 'scale', damage: 1.4, range: 1.3 }),
        tier('Psystrike', 520, 40, 'Shockwaves hit 25% harder and reach 20% further. Unlocks Psystrike.',
          { kind: 'scale', damage: 1.25, range: 1.2 }, { kind: 'signature', signatureId: 'psystrike' }),
      ] },
      { id: 'barrier', label: 'BARRIER', tiers: [
        tier('Amnesia', 160, undefined, 'Towers in range attack 15% faster.',
          { kind: 'rateAura', bonus: 0.15 }),
        tier('Calm Mind', 290, 8, 'Towers in range attack 25% faster.',
          { kind: 'rateAura', bonus: 0.25 }),
        tier('Barrier', 500, 40, 'Every tower in range can aim at Phantoms. Unlocks Mind Link.',
          { kind: 'revealAura' }, { kind: 'signature', signatureId: 'mind_link' }),
      ] },
      { id: 'beam', label: 'BEAM', tiers: [
        tier('Psybeam', 170, undefined, 'The attack becomes a long psychic beam.',
          { kind: 'replaceAttack', moveId: 'psybeam_snipe' }),
        tier('Focus Blast', 300, 8, 'Beams hit Titans and elites 50% harder.',
          { kind: 'bonusVs', target: 'boss', multiplier: 1.5 }),
        tier('Teleport', 510, 40, 'Beams hit 25% harder. Unlocks Teleport.',
          { kind: 'scale', damage: 1.25 }, { kind: 'signature', signatureId: 'mewtwo_teleport' }),
      ] },
    ],
    createModel: () => M.createAlakazam(),
    description: 'A Pokémon made by science to be the strongest there is.',
  },
};
...
const BASE_DEX: Record<string, number> = {
  ...
  weedle: 13, staryu: 120, articuno: 144, zapdos: 145, moltres: 146, mewtwo: 150,
};
