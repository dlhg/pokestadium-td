...
export const SIGNATURES: Record<string, SignatureDef> = Object.fromEntries([
  ...
  def('blaze', 'Blaze', 'Fire', 1, 'Every burning creep on the pitch takes its remaining burn at once.',
    { kind: 'cashDot', effect: 'burn', multiplier: 1 }),
  def('fly', 'Fly', 'Flying', 2, 'Dives on the creep with the most HP for a massive Heavy hit.',
  ...
  def('blizzard', 'Blizzard', 'Ice', 1, 'Freezes every creep in range solid for 3 s.',
    { kind: 'strikeArea', reach: 1, move: hit('blizzard', { basePower: 20, heavy: false, statusEffect: 'stun', statusChance: 1, statusDuration: 3 }) }),
  def('surf', 'Surf', 'Water', 2, 'A wave washes grounded creeps back up the lane.',
  ...
  def('thunder_wave', 'Thunder Wave', 'Electric', 1, 'Paralyzes every creep in range for 4 s. Ground types shrug it off.',
  ...
  // ---- Abra ---------------------------------------------------------------
  def('psychic_storm', 'Psychic', 'Psychic', 1, 'A Heavy psychic blow to every creep on the pitch.',
    { kind: 'strikeArea', reach: 'map', move: hit('psychic', { basePower: 90, heavy: true, delivery: 'aura' }) }),
  def('mass_hypnosis', 'Hypnosis', 'Psychic', 1, 'Every creep on the pitch falls asleep for 4 s.',
    { kind: 'areaStatus', status: 'sleep', duration: 4, reach: 'map' }),
  def('teleport_strike', 'Teleport', 'Psychic', 2, 'Blinks behind the creep with the most HP for a Heavy strike.',
    { kind: 'dive', move: hit('psychic', { name: 'Teleport', basePower: 240, heavy: true }) }),
  // ---- Gastly -------------------------------------------------------------
  def('mass_confuse', 'Confuse Ray', 'Ghost', 1, 'Every creep in wide range stumbles backwards for 4 s.',
  ...
  def('nightmare', 'Nightmare', 'Ghost', 1, 'Every creep in range falls asleep for 4 s.',
    { kind: 'areaStatus', status: 'sleep', duration: 4, reach: 1.2 }),
  // ---- Rattata ------------------------------------------------------------
  def('super_fang', 'Super Fang', 'Normal', 2, 'Bites half the HP off the toughest creep in sight. Bosses lose 10%.',
  ...
  def('whirlwind', 'Whirlwind', 'Flying', 1, 'Blows every creep in wide range back up the lane — flyers too.',
    { kind: 'pushWave', move: hit('gust', { basePower: 30 }), distance: 8, reach: 1.5 }),
  def('sky_attack', 'Sky Attack', 'Flying', 2, 'Dives on the creep with the most HP for a Heavy hit.',
    { kind: 'dive', move: hit('wing_attack', { name: 'Sky Attack', basePower: 220, heavy: true }) }),
  // ---- Zubat --------------------------------------------------------------
  def('swoop', 'Air Cutter', 'Flying', 2, 'Dives on the creep with the most HP for a Heavy hit.',
    { kind: 'dive', move: hit('wing_attack', { name: 'Air Cutter', basePower: 200, heavy: true }) }),
  def('screech', 'Screech', 'Normal', 1, 'Every creep in range stumbles backwards for 3 s.',
    { kind: 'areaStatus', status: 'confuse', duration: 3, reach: 1.4 }),
  def('toxic_wave', 'Toxic', 'Poison', 1, 'Badly poisons every creep in range for 10 s.',
  ...
  def('x_scissor', 'X-Scissor', 'Bug', 2, 'Dives on the creep with the most HP for a Heavy slash.',
  ...
  def('earthquake_ring', 'Earthquake', 'Ground', 1, 'A Heavy quake through every grounded creep in wide range.',
    { kind: 'strikeArea', reach: 1.6, move: hit('earthquake', { basePower: 200, heavy: true, delivery: 'field' }) }),
  def('double_edge', 'Double-Edge', 'Normal', 2, 'Rolls into the creep with the most HP for a massive Heavy hit.',
    { kind: 'dive', move: hit('body_slam', { name: 'Double-Edge', basePower: 260, heavy: true }) }),
  // ---- Machop -------------------------------------------------------------
  def('dynamic_punch', 'DynamicPunch', 'Fighting', 2, 'A crushing Heavy punch on the toughest creep that leaves it confused.',
  ...
  def('bulk_up', 'Bulk Up', 'Fighting', 2, 'Attacks 150% faster for 8 s.',
    { kind: 'selfBoost', bonus: 1.5, duration: 8 }),
  def('submission', 'Submission', 'Fighting', 1, 'Hurls every creep in wide range back up the lane.',
    { kind: 'pushWave', move: hit('seismic_toss', { name: 'Submission', basePower: 60 }), distance: 5, reach: 1.5 }),
  // ---- Onix ---------------------------------------------------------------
  def('rock_wall', 'Rock Wall', 'Rock', 2, 'Raises a wall of stone on the lane that holds creeps for 6 s.',
    { kind: 'dropHazard', hazard: 'rock_wall', radius: 3.5 }),
  def('mass_bind', 'Bind', 'Normal', 1, 'Pins every creep in range in place for 3 s.',
    { kind: 'areaStatus', status: 'stun', duration: 3, reach: 1.2 }),
  def('onix_quake', 'Earthquake', 'Ground', 1, 'A Heavy quake through every grounded creep in range.',
  ...
  def('horn_drill', 'Horn Drill', 'Normal', 1, 'A one-hit knockout on the toughest creep in sight. Bosses lose 12%.',
  ...
  def('volt_wave', 'Thunder Wave', 'Electric', 1, 'Paralyzes every creep in wide range for 4 s. Ground types shrug it off.',
  ...
  def('inferno', 'Inferno', 'Fire', 1, 'Sets every creep in wide range ablaze.',
  ...
  def('stomp', 'Stomp', 'Normal', 2, 'A stamp that throws grounded creeps in range back up the lane.',
  ...
  def('venoshock', 'Venoshock', 'Poison', 1, 'Every poisoned creep takes double its remaining poison at once.',
    { kind: 'cashDot', effect: 'poison', multiplier: 2 }),
  def('sleep_powder_sig', 'Sleep Powder', 'Grass', 1, 'Every grounded creep in wide range falls asleep for 4 s.',
  ...
  def('mass_confusion', 'Confusion', 'Psychic', 1, 'Every creep in wide range stumbles backwards for 4 s.',
    { kind: 'areaStatus', status: 'confuse', duration: 4, reach: 1.5 }),
  ...
  def('glare', 'Glare', 'Normal', 1, 'Paralyzes every creep in wide range for 5 s.',
  ...
  // ---- Lapras -------------------------------------------------------------
  def('sheer_cold', 'Sheer Cold', 'Ice', 1, 'Freezes every creep in wide range solid for 4 s.',
    { kind: 'areaStatus', status: 'stun', duration: 4, reach: 1.5 }),
  def('lullaby', 'Sing', 'Normal', 1, 'Every creep in wide range falls asleep for 5 s.',
  ...
  def('psychic_all', 'Psychic', 'Psychic', 1, 'A psychic blow to every creep on the pitch.',
    { kind: 'strikeArea', reach: 'map', move: hit('psychic', { basePower: 80, delivery: 'aura' }) }),
  def('stun_grove', 'Stun Spore', 'Grass', 1, 'Paralyzes every creep in wide range for 4 s.',
    { kind: 'areaStatus', status: 'paralyze', duration: 4, reach: 1.5 }),
  // ---- Scyther ------------------------------------------------------------
  def('x_scissor_scyther', 'X-Scissor', 'Bug', 2, 'Dives on the creep with the most HP for a massive Heavy slash.',
  ...
  def('tidal_wave', 'Surf', 'Water', 2, 'A tidal wave that washes grounded creeps far back up the lane.',
    { kind: 'pushWave', move: hit('surf', { basePower: 80 }), distance: 8, reach: 1.6, grounded: true }),
  def('roar', 'Roar', 'Normal', 1, 'Every creep in wide range freezes in fear for 3 s.',
