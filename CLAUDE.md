...
- **`web/src/td/StadiumUI.ts`** — *90s Stadium Presentation.* Catch tags over weakened creeps with a ball picker (counts, odds, 1–3 hotkeys) and the CATCH NOW tray (Q cycles). Tower path shop (three species paths with tier pips, level locks, closed/capped warnings and capability chips, earned-evolution read-out, sell footer), the signature bar, metallic tournament headers, 3D floating HP bars, and the team roster deck.
